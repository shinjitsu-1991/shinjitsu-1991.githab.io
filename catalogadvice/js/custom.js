function privacy() {
    var obj = document.getElementById('modalPolitika');
...
$(document).ready(function () {
    ...
    var timeoutToShowStockPopup;
    if (winWidth < 1025) {
        timeoutToShowStockPopup = setTimeout(function () {
            showStockPopup();
        }, 10000);
    }
    ...
    window.mustScrollToCatalog = true;
    ...
    function checkOrderForm(form_id) {
        var result = true;
        var inputName = $(form_id).find("input[name=name]");
        if ($(inputName).length > 0) {
            ...
            if (name == 'Имя' || name == '') {
                ...
                $(form_id).find("input[name=name]").css("border-color", "#f8bb3c");
            }
        }

        var inputPhone = $(form_id).find("input[name=phone]");
        if ($(inputPhone).length > 0) {
            var phone = $(inputPhone).val();
            if (phone == 'Телефон' || phone == '' || phone == '+375(__) ___-__-__') {
                $(inputPhone).css("border", "1px solid red");
                phone = $(inputPhone).val('Телефон');
                result = false;
            } else {
                $(form_id).find("input[name=phone]").css("border-color", "#f8bb3c");
            }
        }
/*        if (result){
            ga('send', 'event','formAny','targetAny','targetAll');
        }*/
        return result;
    }
